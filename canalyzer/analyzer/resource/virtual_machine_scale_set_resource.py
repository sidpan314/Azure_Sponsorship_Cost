...
class VirtualMachineScaleSetResource(Resource):
    def __init__(self, resource_group: ResourceGroup, resource) -> None:
        super().__init__(resource_group, resource)
        self.extra = compute_client.virtual_machine_scale_sets.get(
            self.resource_group.name, self.name
        )
        self.count = self.extra.sku.capacity

    def __is_linux(self):
        return (
            self.extra.virtual_machine_profile.storage_profile.os_disk.os_type
            == "Linux"
        )
    ...
    def price(self):
        ...
        items = retail_price.from_filters(
            ...
            armSkuName=self.extra.sku.name,
        )
        items = [
            x
            for x in items
        ...
        self._price = items[0]["retailPrice"] * self.count
        return self._price
